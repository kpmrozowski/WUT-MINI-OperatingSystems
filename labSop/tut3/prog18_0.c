...
#define DEFAULT_K DEFAULT_N * 15 / 1000
#define BIN_COUNT 11
...
int main(int argc, char **argv)
{
	...
	int bins[BIN_COUNT];
	int threadThrown[throwersCount];
	...
	for (int i = 0; i < throwersCount; i++)
	{
		...
		args[i].bins = bins;
		args[i].pthreadThrown = threadThrown;
	...
	printf("Threads' throw counts:\n");
	for (int i = 0; i < throwersCount; i++)
		printf("%d ", threadThrown[i]);
...
void *throwing_func(void *voidArgs)
{
	...
	while (1)
	{
		...
		args->pthreadThrown[args->number] += 1;
	}
	return NULL;
}
